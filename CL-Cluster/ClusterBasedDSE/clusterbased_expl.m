...
trainCondition = py.Assistant.TrainCondition(1,py.None);
...
while clusterize
    ...
    [paretoPoints, paretoIdx]  = paretoFrontier(sampledData);
    is_promising = 1;
    classifier = py.None;
    for p=1:size(intraclusterExplorationFeatures,1)
        ...
        pareto_frontier = py.numpy.array(sampledData(paretoIdx,:));
        history = py.numpy.array(sampledData);
        new_configuration = py.numpy.array(data(indx,3:end)).tolist();
        [is_promising,trainCondition] = encapsulation(sampledData,data(indx,3:end),sampledData(paretoIdx,:),is_promising,trainCondition);
        if double(is_promising) == 1
            newData = [newData; data(indx,1), data(indx,2), data(indx,3:end)];
            
        else
            newData = [newData; 10000000, 10000000, data(indx,3:end)];
        end
        ...
        if double(is_promising)==1
            [paretoPoints, paretoIdx]  = paretoFrontier(sampledData);
            onlineADRS = [onlineADRS; ADRS(paretoRealData, paretoPoints)];
        end
    ...
    for p=1:size(interclusterExplorationFeatures,1)
        ...
        pareto_frontier = py.numpy.array(sampledData(paretoIdx,:));
        history = py.numpy.array(sampledData);
        new_configuration = py.numpy.array(data(indxInterclustering,3:end)).tolist();
        [is_promising,trainCondition] = encapsulation(sampledData,data(indxInterclustering,3:end),sampledData(paretoIdx,:),is_promising,trainCondition);
        if double(is_promising) == 1
            newData2 = [newData2; data(indxInterclustering,1), data(indxInterclustering,2), data(indxInterclustering,3:end)];
        else
            newData2 = [newData2; 10000000, 10000000, data(indxInterclustering,3:end)];
        end
